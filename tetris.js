//Copyright (C) 2014 Aydin Gungordu - www.aydingungordu.com
//Tetris game v0.01
...
function canRotate() {
    ...
    var nextState = (go.state + 1) % go.states.length;

    for (var i = 0; i < go.states[nextState].length; i++) {
        var x = go.states[nextState][i].x;
        var y = go.states[nextState][i].y;

        if (TETRIS.myMatrix[x][y] !== 0 && TETRIS.myMatrix[x][y] !== go.id)
            return false;
    }
    return true;
}
